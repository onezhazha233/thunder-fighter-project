...
for(i=0;i<1;i+=1){
	if(!instance_exists(laser[i])){
		...
		laser[i].SetSequence(seq_bullet_player_falcon_b_fire);
	...
	else if(laser[i].sequence != seq_bullet_player_falcon_b_fire){
		laser[i].SetSequence(seq_bullet_player_falcon_b_fire);
		laser[i].width = 60;
		laser[i].range = 500;
		laser[i].rampage = false;
...
if(ring_time mod 10 = 0){
	...
	ring.sequence = seq_bullet_player_falcon_b_ring;
