...
if(keyboard_check_pressed(ord("E"))){
	if!(layer_sequence_get_sequence(s) = sequence_get(seq_phantom_b_rampage)){
		layer_sequence_destroy(s);
		s = layer_sequence_create("planes",x,y,seq_phantom_b_rampage_intro);
...
if(layer_sequence_get_sequence(s) = sequence_get(seq_phantom_b_rampage_intro)&&layer_sequence_is_finished(s)){
	if(layer_sequence_get_headdir(s) = seqdir_right){
		layer_sequence_destroy(s);
		s = layer_sequence_create("planes",x,y,seq_phantom_b_rampage);
		layer_sequence_xscale(s,3);
		layer_sequence_yscale(s,3);
	}
	else{
