live;
laser_scale_target = 1.7

for(i=0;i<1;i+=1){
	if(!instance_exists(laser[i])){
		laser[i] = MakePlayerBullet(x,y,bullet_player_plane_falcon);
		laser[i].SetSequence(seq_bullet_player_falcon_d_fire_rampage);
	...
	else if(laser[i].sequence != seq_bullet_player_falcon_d_fire_rampage){
		laser[i].SetSequence(seq_bullet_player_falcon_d_fire_rampage);
		laser[i].width = 180;
		laser[i].range = 1200;
		laser[i].rampage = true;
...
if(ring_time mod 12 = 0){
	...
	ring.sequence = seq_bullet_player_falcon_d_ring_rampage;
	ring.image_xscale = scale_x*0.75;
	ring.image_yscale = laser_scale*0.6;
	array_push(follow_inst,ring);
}

if!(audio_is_playing(snd_plane_falcon_rampage))SFX_Play(snd_plane_falcon_rampage,0,true);