...
if(keyboard_check_pressed(ord("E"))){
	if!(layer_sequence_get_sequence(sr) = sequence_get(seq_shotcannon_a_rampage)){
		sl = layer_sequence_create("planes",x-ox,y+oy,seq_shotcannon_a_rampage_intro);
		layer_sequence_xscale(sl,-3);
		layer_sequence_yscale(sl,3);
		sr = layer_sequence_create("planes",x+ox,y+oy,seq_shotcannon_a_rampage_intro);
...
if(layer_sequence_get_sequence(sr) = sequence_get(seq_shotcannon_a_rampage_intro)&&layer_sequence_is_finished(sr)){
	if(layer_sequence_get_headdir(sr) = seqdir_right){
		layer_sequence_destroy(sl);
		layer_sequence_destroy(sr);
		sl = layer_sequence_create("planes",x-ox,y+oy,seq_shotcannon_a_rampage);
		layer_sequence_xscale(sl,-3);
		layer_sequence_yscale(sl,3);
		sr = layer_sequence_create("planes",x+ox,y+oy,seq_shotcannon_a_rampage);
		layer_sequence_xscale(sr,3);
		layer_sequence_yscale(sr,3);
	}
	else{
