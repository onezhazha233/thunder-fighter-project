...
if(instance_exists(find_enemy)||room != room_battle){
	...
	if(edge_state = 2){
		...
		if(edge_attack_sequence_enabled = true){
			if(rampage = true){
				edge_attack_sequence = layer_sequence_create(layer_edge,edge_x,edge_y,seq_bullet_player_blade_a_edge_attack_rampage);
				SFX_Play(snd_plane_blade_shoot_rampage);
			}
			else{
				edge_attack_sequence = layer_sequence_create(layer_edge,edge_x,edge_y,seq_bullet_player_blade_a_edge_attack);
