...
for(i=0;i<1;i+=1){
	if(!instance_exists(laser[i])){
		...
		laser[i].SetSequence(seq_bullet_player_falcon_c_fire_rampage);
	...
	else if(laser[i].sequence != seq_bullet_player_falcon_c_fire_rampage){
		laser[i].SetSequence(seq_bullet_player_falcon_c_fire_rampage);
		laser[i].width = 180;
		laser[i].range = 1200;
		laser[i].rampage = true;
...
if(ring_time mod 12 = 0){
	...
	ring.sequence = seq_bullet_player_falcon_c_ring_rampage;
