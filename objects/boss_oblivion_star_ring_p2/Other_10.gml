...
SetPosition = function(xx,yy){
	...
	for(i=0;i<4;i+=1){
		...
		if(layer_sequence_exists(variable_instance_get(id,"layer_enemy_"+string(i)),eseq)){
			...
			if(i = 0){
				layer_sequence_angle(eseq,corner_angle);
			}
			else if(i = 2){
				...
				layer_sequence_x(eseq,x-surf_x*surf_enabled+gun_xoff);
			}
			else{
...
SetSurfEnabled = function(enabled){
	...
	l = variable_instance_get(id,"layer_enemy_3");
	layer_script_begin(l,scrBegin);
...
SetIdle = function(duration=30){
	...
	Anim_Destroy(id,"gun_xoff");
	Anim_Destroy(id,"gun_angle");
	Anim_Destroy(id,"corner_angle");
	ring_angle = ring_angle mod 120;
	corner_angle = corner_angle mod 120;
	Anim_Create(id,"ring_angle",ANIM_TWEEN.QUAD,ANIM_EASE.IN_OUT,ring_angle,-ring_angle,duration);
	Anim_Create(id,"gun_xoff",ANIM_TWEEN.QUAD,ANIM_EASE.IN_OUT,gun_xoff,-gun_xoff,duration);
	Anim_Create(id,"gun_angle",ANIM_TWEEN.QUAD,ANIM_EASE.IN_OUT,gun_angle,angle_difference(0,gun_angle),duration);
	Anim_Create(id,"corner_angle",ANIM_TWEEN.QUAD,ANIM_EASE.IN_OUT,corner_angle,-corner_angle,duration);
...
SetFrozen = function(f){
    ...
    if(f > 0){
        ...
        layer_sequence_speedscale(enemy_sequence_0,0);
        layer_sequence_speedscale(enemy_sequence_3,0);
        SetFlame(-1,-1);
        ...
        layer_sequence_speedscale(enemy_sequence_0,1);
        layer_sequence_speedscale(enemy_sequence_3,1);
...
SwapSequence = function(idx,seq){
	var _l = variable_instance_get(id,"layer_enemy_"+string(idx));
	var _old = variable_instance_get(id,"enemy_sequence_"+string(idx));
	var _new = layer_sequence_create(_l,x,y,seq);
	layer_sequence_xscale(_new,image_xscale);
	layer_sequence_yscale(_new,image_yscale);
	variable_instance_set(id,"enemy_sequence_"+string(idx),_new);
	layer_sequence_destroy(_old);
	SetPosition(x,y);
}

GunAttack = function(count=1){
	gun_attack_count = max(1,count);
	gun_attack_played = 0;
	gun_attack_active = true;
	SwapSequence(3,seq_enemy_boss_large_og2_p2_idle_3_attack);
	SwapSequence(0,seq_enemy_boss_large_og2_p2_idle_0_flash);
}