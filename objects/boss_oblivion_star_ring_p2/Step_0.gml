live;
for(i=0;i<4;i+=1){
	if!(layer_exists(variable_instance_get(id,"layer_enemy_"+string(i)))){
		variable_instance_set(id,"layer_enemy_"+string(i),layer_create(depth-i));
	}
	layer_depth(variable_instance_get(id,"layer_enemy_"+string(i)),depth-i);
}
...
if(layer_sequence_is_finished(enemy_sequence_0)){
...
if(gun_attack_active = true){
	if(layer_sequence_is_finished(enemy_sequence_3)){
		gun_attack_played += 1;
		if(gun_attack_played >= gun_attack_count){
			gun_attack_active = false;
			SwapSequence(3,variable_instance_get(id,"idle_sequence_3"));
			SwapSequence(0,variable_instance_get(id,"idle_sequence_0"));
		}
		else{
			layer_sequence_headpos(enemy_sequence_3,0);
			layer_sequence_headpos(enemy_sequence_0,0);
		}
	}
}
