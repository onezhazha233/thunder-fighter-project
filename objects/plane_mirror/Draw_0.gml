...
if(idle_mode = 0){
	if(sprite_exists(idle_sprite)){
		if(draw_plane = 1){
			...
			//draw_sprite_ext(idle_sprite,0,room_width-(x-surf_x*surf_enabled),y-surf_y*surf_enabled,(state == 0&&roll_state != 0 ? 0 : image_xscale),image_yscale,image_angle,image_blend,image_alpha);
			...
			if(state = 0){
				if(roll_state < 0){
					//draw_sprite_ext(sprite_roll_left,clamp(ceil(abs(roll_state))-1,0,2),room_width-(x-surf_x*surf_enabled),y-surf_y*surf_enabled,image_xscale,image_yscale,image_angle,image_blend,image_alpha);
					...
					if(sprite_roll_flip = false){
						//draw_sprite_ext(sprite_roll_right,clamp(ceil(roll_state)-1,0,2),room_width-(x-surf_x*surf_enabled),y-surf_y*surf_enabled,image_xscale,image_yscale,image_angle,image_blend,image_alpha);
						...
						//draw_sprite_ext(sprite_roll_left,clamp(ceil(roll_state)-1,0,2),room_width-(x-surf_x*surf_enabled),y-surf_y*surf_enabled,-image_xscale,image_yscale,image_angle,image_blend,image_alpha);
...
if(surf_enabled = 1){
	...
	draw_surface_ext(surf,room_width-surf_x,surf_y,-1,1,0,make_color_rgb(128,128,128),0.5);
