live;
laser = MakePlayerBullet(x-50,y,bullet_player_plane_glacier);
laser.image_angle = 125;
laser.duration = 26;
laser.start_sprite = spr_bullet_player_glacier_d_start;
laser.glow_offset = 0;
laser.damage = damage
Anim_Create(laser,"image_angle",ANIM_TWEEN.QUAD,ANIM_EASE.OUT,125,-35,20);
Anim_Create(laser,"image_xscale",0,0,1,-0.8,10,16);
array_push(follow_inst,[laser,-50,0]);

laser = MakePlayerBullet(x+50,y,bullet_player_plane_glacier);
laser.image_angle = 55;
laser.duration = 26;
laser.start_sprite = spr_bullet_player_glacier_d_start;
laser.glow_offset = 0;
laser.damage = damage
Anim_Create(laser,"image_angle",ANIM_TWEEN.QUAD,ANIM_EASE.OUT,55,35,20);
Anim_Create(laser,"image_xscale",0,0,1,-0.8,10,24);
array_push(follow_inst,[laser,50,0]);

audio_play_sound(snd_plane_glacier_laser_small,0,0);